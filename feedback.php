...
$sql = "SELECT name, email, num, message FROM feedback";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // output data of each row
   echo "<table border = 1 cellpadding = 10>";
   echo "<tr>" . "<th>Name: </th>" . " <th>Email </th>" .  " <th>Number</th> " . " <th>Message</th></tr> ".  "<br>";
    while($row = $result->fetch_assoc()) {
        
        echo "<tr><td>" . $row["name"]. "</td><td>" . $row["email"]."</td><td>". $row["num"]."</td><td>". $row["message"]."</td> </tr>". "<br>";
